import { insertFilters } from '../insertFilters';
import { COMPARATORS } from '../ComparatorList';
import { OPERATORS } from '../OperatorList';
import { Action } from '../Action';
export class AddFilters extends Action {
	/**
	...
	perform = async (params, r) => {
		...
		for (const filter of params.filters) {
			const trans: any = {};
			if (typeof filter.name === 'string') trans.name = filter.name;
			if (typeof filter.id === 'string') trans.id = filter.id;
			if (typeof filter.code === 'string') {
				...
				function recur(tree, current?) {
					...
					if (
						!top &&
						['number', 'string'].includes(typeof tree[current])
					) {

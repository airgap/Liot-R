...
export async function actionPushUpdate(params, r) {
    ...
    if (newUpdate.id) {
        return await updated(await r.branch(
            r.table('Collectors')
        ...
        return await updated(await r.table('Collectors')
            .getAll(newUpdate.accessor, {index: "accessor"})
            .update({value: newUpdate.value}, {return_changes: 'always'})
        );
        /*r.table('Collectors')
    ...
    async function updated(updated) {
        if (updated && typeof updated.changes[0].new_val == 'object') {
            ...
            const rows = await r.table('Distributors')
                .without('name')
                .merge(doc => ({
                        collators:
                            r.table('Collators')
                    ...
                    for (const filter of collator.filters)
                        if (filters[filter.id]) {
                            ...
                            if (distributor.callback && validUrl.isUri(distributor.url)) {
                                ...
                                try {
                                    const inserted = await r.table('DistributedData').insert(distributedData);
...
function getProperty(object, key) {
    for (const k of key.substring(1).split('.')) {
        const sub = object[k];
        console.log(k, sub)
        if (['undefined', 'sub'].includes(typeof sub)) object = sub;
        else return null;
    }

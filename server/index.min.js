...
var COMPARATORS="EQUALS NEQUALS TFNEQUALS OVER OVEROR UNDER UNDEROR".split(" "),OPERATORS="AND OR NAND NOR XOR SAME".split(" "),r=require("rethinkdb"),express=require("express"),bodyParser=require("body-parser"),validUrl=require("valid-url"),app=express(),fs=require("fs"),CONFIG={port:7474};app.use(bodyParser.json());app.use(function(b,d,a){d.header("Access-Control-Allow-Origin","*");d.header("Access-Control-Allow-Headers","Origin, X-Requested-With, Content-Type, Accept");a()});
...
c+e).merge(function(a){return{collets:r.table("Collators").getAll(r.args(a("collators"))).coerceTo("array")}}).coerceTo("array").run(CONNECTION,function(a,b){a?(d.send({err:"Unable to query."}),console.log(a)):(d.send({distributors:b}),console.log("Queried distributors."),console.log(b))}))})}
...
function actionAddDistributors(b,d,a){if(Array.isArray(a.distributors)){b=[];a=$jscomp.makeIterator(a.distributors);for(var c=a.next();!c.done;c=a.next()){var c=c.value,e={collators:[]};if(Array.isArray(c.collators))for(var f=$jscomp.makeIterator(c.collators),g=f.next();!g.done;g=f.next())e.collators.push(g.value);"string"===typeof c.url&&validUrl.isUri(c.url)&&(e.url=c.url);"string"===typeof c.name&&0<c.name.length&&(e.name=c.name);e.push=!!c.push;e.queue=!!c.queue;e.callback=!!c.callback;b.push(e)}r.table("Distributors").insert(b).run(CONNECTION,
function(a,b){a?(d.send({err:"Error creating distributors."}),console.log(a)):(d.send({}),console.log("Created distributors."))})}}
...
function actionPushUpdate(b,d,a){function c(a,b){}var e;"string"!=typeof a.accessor?d.send({err:"No accessor specified."}):(b=typeof a.value,"undefined"==b||"null"==b?d.send({err:"No value specified."}):"string"==typeof a.id?e=a.id:(d=a.accessor,b=a.value,idsAndAccessors.push({id:a.id,accessor:a.accessor}),r.expr(idsAndAccessors),e?r.branch(r.table("Collectors").getAll(d,{index:"accessor"}).filter({aggregate:!0}).limit(1).count().eq(1),r.table("Collectors").get(e).update({value:b},{kRETURN_CHANGES:kRETURN_CHANGES}),
{replaced:0}).run(CONNECTION,c):r.table("Collectors").getAll(d,{index:"accessor"}).update({value:b},{kRETURN_CHANGES:kRETURN_CHANGES}).run(CONNECTION,c),r.table("Collectors").filter(function(a){})))};

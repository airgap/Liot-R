...
const adminActions = {
	...
	'count filter references':
		actions.filters.references.count,
	...
	'count collator references':
		actions.collators.references.count,
	/*Distributors*/
...
async function launchHttpServer() {
	app.post('/', rcvdPost);
	console.log(
		`HTTP server launched on port ${CONFIG.port}.`
	);
	...
	if (await exists('./webui')) {
		...
		console.log(
			'No admin panel detected. Running in headless mode.'
		);
}
...
async function rcvdPost(req, res) {
	...
	if (isLocal && action in adminActions)
		...
		res.send(publicActions[action](json, r));
}

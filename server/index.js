...
function startRethinkServer() {
  /* 1.db.yup.place => localhost */
        ...
        .run(conn, (err, status) => {
          if (err || !conn) {
            ...
            if (status.dbs_created)
              ...
              else if (DEBUG) console.log("Found database for Liot R.");
            if (DEBUG) console.log("Connected to database for Liot R.");
            for (var t of TABLES) ensureTable(t);
            return true;
          }
        });
    }
...
function ensureTable(t) {
  r.tableList()
    .contains(t)
    .do((e) => {
      return r.branch(e, { dbs_created: 0 }, r.tableCreate(t));
    })
...
function confirmRunning() {
  if (CHECKS.tablesPrepped && CHECKS.httpServerRunning) {
    var out =
      "Liot R launched. Ready for queries" +
      (CHECKS.adminPanelRunning ? " and GUI interaction." : ".");
    console.log(out);
  }
}

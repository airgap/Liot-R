...
function namify(a,b,d){var f=xss(a.name||""),e=xss(a.id);a="<a class='title-edit"+(d?" disabled":"")+"' href=\"edit-"+b+".php?id="+a.id+'">edit'+(d?"ing":"")+"</a>";return(f.length?f:"<i>Unnamed "+b[0].toUpperCase()+b.substr(1)+"</i>")+a+" <span class='name-id'>["+e+"]</span>"}
...
'<span class="variable">$1</span>').replace(/([:,]) (true|false)(,|\n| \])/g,'$1 <span class="binary">$2$3</span>')}function appendDropper(a){var b=nelem("div");b.innerHTML="V";addc(b,"dropper");bind(a.children[0],"click",function(b){togc(a,"dropped");return!1});a.appendChild(b)}function bindBubble(a,b){bind(a,"click",function(){location.href="edit-filter.php?id="+b.id})};

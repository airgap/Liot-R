...
fs.readFile("weave.json", "utf-8", (err, bod) => {
  ...
  CONF.t = CONF.o;
  for (var k of ["i", "o", "t"]) CONF[k] = pather.resolve(CONF[k]);
  //console.log(CONF)
  getArgs();
  listTree(CONF.i);
});
...
function listTree(directory) {
  ...
  fs.readdir(directory, (err, items) => {
    if (!Array.isArray(items)) {
      throw "Unable to read input directory.";
    }
    ...
    function processFile(err, body) {
      ...
      if (ext && CONF.e.includes(ext[1])) {
        ...
        while ((m = e.exec(body))) {
          ...
          DEPENDENCY_COUNT++;
          if (!dependencies.includes(pathObj)) dependencies.push(pathObj);
        }
      }
        ...
        function interpolateDependency(d) {
          ...
          fs.readFile(d.outputPath, "utf-8", (err, bod) => {
            ...
            fContents = fContents
              .replace(
                reg,
                "$1$2" + bod.replace(/\$/g, "KSFDJKDJIEJIKDJFKJEIFJKDJIEJFK")
              )
              .replace(/KSFDJKDJIEJIKDJFKJEIFJKDJIEJFK/g, "$");
            //well.......it works.......
            reqs--;
            if (!reqs) {
              writeReplaced();
...
function outify(d) {
  ...
  var out = d;
  if (CONF.i == d.substring(0, CONF.i.length))
    out = pather.join(CONF.o, d.substring(CONF.i.length + 1));
    ...
    if (filesWrittenInPass) {
      ...
      for (var k of Object.keys(UNMET_DEPENDENCIES)) {
        if (UNMET_DEPENDENCIES[k].length) {
          failed = true;
          t += k.match(/([\/\\].+)/)[1] + ":";
          for (var d of UNMET_DEPENDENCIES[k]) t += "\n->\t" + d.name; //match(/([\/\\].+)/)[1];
          t += "\n";
        }
      }
      if (failed) {

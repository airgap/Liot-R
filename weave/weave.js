...
fs.readFile('weave.json', 'utf-8', (err, bod) => {
	if (bod) {
		...
		if (fileConfig.directories) {
			...
			CONF.o =
				fileConfig.directories.output || CONF.o;
		...
		if (!UNMET_DEPENDENCIES[f].length) {
			...
			function interpolateFile(f) {
							...
							fContents = fContents
								.replace(
									...
									'$1$2' +
										bod.replace(
											/\$/g,
											'KSFDJKDJIEJIKDJFKJEIFJKDJIEJFK'
										)
								)
								.replace(
									/KSFDJKDJIEJIKDJFKJEIFJKDJIEJFK/g,
									'$'
								);
							//well.......it works.......
							reqs--;
				...
				function writeReplaced() {
					...
					var derp =
						outify(f).match(/(.+[\/\\])/)[1]; //(f.match(/(.+\/)/)||[null,])[1]);
					mkdirp(derp, err => {
						if (err) throw err;

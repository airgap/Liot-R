...
function grab(elem) {
	if (typeof elem === 'string') {
		...
		elem.split('|').forEach(x =>
			o.push(document.getElementById(x))
		);
...
function bind(elem, evt, func) {
	iterate(grab(elem), x => {
		...
		if (!bind.bound[x]) bind.bound[x] = [];
		bind.bound[x].push(func);
	});
}
...
function pang(path, callback) {
	...
	x.open(
		'GET',
		imgroot + 'pangpacks/' + 'frames.pangpack'
	);
	bind(x, 'load', () => {
...
if (typeof BS == 'undefined') {
	BS = zone => {
		...
		var place = (
			zone || document
		).getElementsByClassName('bs-textbox');
		for (var bi = 0; bi < place.length; bi++) {
			...
			function bindbox(box) {
				var correct =
					box.getAttribute('bs-correct');
				var incorrect =
					box.getAttribute('bs-incorrect');
